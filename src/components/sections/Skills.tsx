...
function NetworkDiagram() {
  ...
  useEffect(() => {
    ...
    lines.forEach((ln, i) => {
      ...
      setTimeout(() => {
        ln.style.transition = "stroke-dashoffset .7s cubic-bezier(.4,0,.2,1)"
        ln.style.strokeDashoffset = "0"
      }, i * 70)
    })
    const nodesG = svgRef.current.querySelector<SVGGElement>("#diag-nodes")
    if (nodesG) setTimeout(() => { nodesG.style.opacity = "1" }, lines.length * 70 + 200)
